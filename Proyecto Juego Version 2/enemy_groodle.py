...
class SpiritGroodle(StillShooter):
    ...
    def determinar_direccion(self):
        if self.direccion == DERECHA:
            self.rect_vision = pygame.Rect(self.rect.x - 20 + self.rect.w, self.rect.y, 400, 100)
            self.velocidad = 15
        else:
            self.rect_vision = pygame.Rect(self.rect.x - 270 - self.rect.w, self.rect.y, 400, 100)
            self.velocidad = -15
