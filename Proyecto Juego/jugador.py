...
class Jugador:
    def __init__(self, pos_x, pos_y, screen) -> None:
        ...
        self.health_bar = ProgressBar(master=screen,x=10,y=10,w=350,h=20,color_background=M_BRIGHT_CLICK,color_border=C_WHITE, value_max = self.vida)
        self.layer_orb = Widget(master_form=screen, x=10, y = 40, w=25, h=25, color_background= None, color_border=None, image_background=r"Proyecto Juego\recursos\caracters\stink\disparo_item.png",text = None,font=None,font_size=None,font_color=None)
        self.layer_ammo = Widget(master_form=screen, x=50, y = 40, w=50, h=25, color_background= C_CELEST, color_border=C_BLUE, image_background=None,text="0",font="Arial",font_size=25,font_color=C_BLUE_2)
        self.layer_score = Widget(master_form=screen, x=1090, y = 10, w=100, h=30, color_background=C_YELLOW_2, color_border=C_BROWN, image_background=None, text="0", font="Arial", font_size=30, font_color=C_BLACK)
        self.lista_widget = [self.health_bar, self.layer_orb, self.layer_ammo, self.layer_score]
    ...
    def modificar_vida(self,atacante):
        ...
        print("Puntos de vida: {0}".format(self.vida))
        if self.vida < 1:
            self.vivo = False
    ...
    def actualizar(self, plataformas, objetivos, delta_ms, lista_eventos, teclas_presionadas):
        if self.activo:
            ...
            if self.timer > 30:
                ...
                self.health_bar.value = self.vida
